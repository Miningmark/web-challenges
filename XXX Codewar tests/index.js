...
function lastDigit(arr) {
    if (arr.length === 0) {
        return 1; // Empty list, return 1 as specified
    }

    let result = 1;

    for (let i = arr.length - 1; i >= 0; i--) {
        // Use modular exponentiation to avoid large intermediate values
        result = modularExponentiation(arr[i], result, 10);
    }
...
function modularExponentiation(base, exponent, modulus) {
    if (modulus === 1) return 0; // In case modulus is 1
