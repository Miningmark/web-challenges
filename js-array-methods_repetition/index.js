...
const recipesThatOnlyTake60minutesOrLess = recipes.filter((recipe) => recipe.duration <= 60).map((recipe) => recipe.title).join(', ');
console.log(recipesThatOnlyTake60minutesOrLess);

const allRecipesWithMoreThan2Servings = recipes.filter((recipe) => recipe.servings > 2).map((recipe) => recipe.title).join(', ');
console.log(allRecipesWithMoreThan2Servings);

const allRecipesWithTitlesLongerThan12Characters = recipes.filter((recipe) => recipe.title.length > 12).map((recipe) => recipe.title).join(', ');
